    ...
    switch(medTab2){
      ...
      case "prontuario": return(
        <div>
          ...
          {["evolucao","drive","exames"].includes(pronTab2)&&<div><div style={{display:"flex",justifyContent:"flex-end",marginBottom:8}}><button onClick={()=>{if(window.confirm("Limpar atendimento?")){const np={...PAC0,pacID:genPacID()};setPac(np);clearPacAtual?.();setDossieOncologico(criarDossieInicial(np));}}} style={{background:"none",border:"1px solid #DDE3EC",borderRadius:8,padding:"5px 12px",fontSize:11,color:"#94A3B8",cursor:"pointer",fontFamily:"inherit"}}>🗑 Limpar</button></div><ProntuarioDossieUnico pac={pac} dossie={dossieOncologico} setDossie={setDossieGuardado} up={up} addMsg={addMsg} onAbrirAPAC={()=>setMedTab2("apac")} onSalvarEvolucao={(evolucao)=>{if(!executarProntuarioSecurity({pac,texto:evolucao,dossie:dossieOncologico,origem:"Salvar evolução do dossiê"},addMsg))return false;const dt=new Date();const nv={id:Date.now(),data:dt.toLocaleDateString("pt-BR"),hora:dt.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),tipo:"Consulta médica",texto:evolucao,autor:"Dr. Silas Negrão — CRM-PB 17341",fonte:"dossie"};const ne=[nv,...(pac?.evolucoes||[])];up("evolucoes",ne);dbSalvarPaciente({...pac,evolucoes:ne});addMsg&&addMsg("Médico","Equipe","✅ Evolução salva.","msg");return true;}}/></div>}
